...
//1920/8
#define PIXEL_USHORT8_WIDTH  240
...
__kernel void kernel_bayer_copy (
...
)
{
    ...
    const int l_x = get_global_id (0);
    const int l_y = get_global_id (1);
...
#else
        line1 = convert_float8 (input [y * 2 * PIXEL_USHORT8_WIDTH + x]) / 65536.0f;
        line2 = convert_float8 (input [(y * 2 + 1) * PIXEL_USHORT8_WIDTH + x]) / 65536.0f;
#endif
...
#endif
    write_imageui (output, (int2)(x, y), as_uint4 (convert_ushort8 (data_gr * 65535.0f)));
    write_imageui (output, (int2)(x, y), as_uint4 (convert_ushort8 (data_r * 65535.0f)));
    write_imageui (output, (int2)(x, y), as_uint4 (convert_ushort8 (data_b * 65535.0f)));
    write_imageui (output, (int2)(x, y), as_uint4 (convert_ushort8 (data_gb * 65535.0f)));
