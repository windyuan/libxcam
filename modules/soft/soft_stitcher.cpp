...
FisheyeDewarp::set_dewarp_geo_table (SmartPtr<SoftGeoMapper> mapper, const CameraInfo &cam_info, const BowlDataConfig &bowl)
{
    ...
    for (uint32_t row = 0; row < table_height; row++) {
        for(uint32_t col = 0; col < table_width; col++) {
            ...
            data.x = map_table[row * table_width * 2 + col * 2] / (float)cam_info.slice_view.width;
            data.y = map_table[row * table_width * 2 + col * 2 + 1] / (float)cam_info.slice_view.height;
            data.z = 0.0f;
            data.w = 0.0f;
        }
    }
    ...
    const XCamReturn error)
{
    ...
    //dump_buf (dewarp_param->out_buf, "dewarp", dewarp_param->idx);
